...
CREATE TABLE `Booking Detail` (
    ...
    CONSTRAINT `fk_bookingdetail_tickettype` FOREIGN KEY (`ticket_type_id`) REFERENCES `Ticket Type` (`ticket_type_id`)
);
...
-- JOIN `Ticket Type` USING (`ticket_type_id`) ;
...
CREATE TABLE `Ticket` (
    ...
    CONSTRAINT `fk_ticket_tickettype` FOREIGN KEY (`ticket_type_id`) REFERENCES `Ticket Type` (`ticket_type_id`)
);
...
DELIMITER //
CREATE TRIGGER `after_insert_checkout`
AFTER INSERT ON `Checkout`
FOR EACH ROW
BEGIN
	UPDATE `Booking` 
	SET `status` = 'completed'
    WHERE NEW.`booking_id` = `booking_id`;
END //

DELIMITER //
CREATE TRIGGER `after_insert_transaction_detail`
AFTER INSERT ON `Transaction Detail`
FOR EACH ROW
BEGIN
	SELECT `ticket_type_id` INTO @type_id
    FROM `Ticket` 
...
-- JOIN `Checkout` USING (`checkout_id`)
-- JOIN `Ticket` USING (`ticket_id`)


-- Test Trigger when Checkout
-- INSERT INTO `Checkout`
-- VALUES (3,'EZ83495254', 0.08, 1944000, DEFAULT, DEFAULT);
